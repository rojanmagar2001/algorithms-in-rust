...
impl<T> Stack<T> {
    ...
    // The push method wich the signature's first parameter is self
...
///
/// Here's a question: *Do we need to worry about cleaning up out list?*\
/// With the help of the ownership mechanish, the type `List` will be cleaned up automatically (dropped) after it goes out of scope.\
/// The Rust Compiler does so automatically. In other words, the `Drop` trait is implemented automatically.\
///
...
///
/// We wouldn't be able to drio the contents contained by the box after deallocating, so we need to manually write the iterative drop.
...
// Rust has nothing like a yeild statement, and there are actually 3 different iterator traits to be implemented
...
impl<'a, T> Iterator for Iter<'a, T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.next.map(|node| {
            // as_deref: Convers from Option<T> (or &Option<T>) to Option<&T::Target>.
